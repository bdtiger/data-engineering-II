...
Using Docker is quick and easy. Pulsar can be started with one command and it works the
...
Setting \texttt{PULSAR\_ADVERTISED\_ADDRESS} fixes this. There is also a small
...
However, Java, ZooKeeper, BookKeeper, and Pulsar must all be installed manually, which
...
\begin{itemize}[leftmargin=1.5cm]
  \item \textbf{Unified messaging}: Pulsar supports both message queuing and publish-subscribe in
...
Pulsar uses the \textbf{publish-subscribe} pattern. A producer sends messages
...
The main advantage is that producers and consumers are loosely coupled. Consumers can
be added or removed without changing the producer. Multiple consumers can also read from
...
\begin{itemize}[leftmargin=1.5cm]
  \item \textbf{Exclusive}: only one consumer is allowed at a time. Used when strict
        ordering and a single consumer are required.
  \item \textbf{Shared}: messages are spread across multiple consumers. Used when
...
running, which broker owns which topic, and cluster configuration.

Yes, it is possible to ensure reliability without ZooKeeper. 
Pulsar 2.10 and later introduced a pluggable metadata store, allowing ZooKeeper to be replaced with an embedded metadata store. 
However, some coordination service is always needed. Without one, the cluster cannot reliably agree on its state.
...
\begin{itemize}[leftmargin=1.5cm]
  ...
  \item \textbf{BookKeeper}: store messages on disk. Multiple bookies replicate
  ...
  \item \textbf{Pulsar Functions}: run small processing tasks directly inside Pulsar.
\end{itemize}
...
A Pulsar tenant is an administrative unit that represents a team, company, or
...
When a consumer finishes processing a message, it sends an acknowledgement to the
broker. This tells the broker the message was handled successfully and can be removed.
If the consumer does not send an acknowledgement, the broker will
redeliver the message after a timeout. This ensures that every message will be processed at least once. 
A consumer can also send a negative
acknowledgement to ask the broker to resend a message right away. Without
acknowledgements, the broker cannot tell whether a message was processed.
